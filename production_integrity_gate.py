...
def main() -> None:
    if not RESULTS.exists():
        raise SystemExit("INTEGRITY_FAIL: results directory missing")
    files = [p for p in RESULTS.rglob("*") if p.is_file() and p.stat().st_size > 0]
    csvs = [p for p in files if p.suffix.lower() == ".csv"]
    if not csvs:
        raise SystemExit("INTEGRITY_FAIL: no non-empty CSV results")

    checks = {"csv_nonempty": True, "probabilities_valid": True, "duplicate_rows": True}
    total_rows = 0
    probability_columns = []
    for path in csvs:
        with path.open("r", encoding="utf-8", errors="replace", newline="") as f:
            ...
            if not fields:
                raise SystemExit(f"INTEGRITY_FAIL: CSV has no header: {path}")
            rows = list(reader)
        if not rows:
            raise SystemExit(f"INTEGRITY_FAIL: CSV has no data rows: {path}")
        total_rows += len(rows)
        prob = [c for c in fields if c.lower().endswith("prob") or "probability" in c.lower()]
        for c in prob:

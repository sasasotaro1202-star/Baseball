...
pool. It creates one chronological Development OOS comparison for the current
production ensemble versus individual model candidates, locks the winner using
Development OOS only, then evaluates that locked candidate on a later, untouched
holdout window. The holdout is never read during candidate selection.
"""
...
def _target_metrics(bt: BaseballBacktest, X_train: pd.DataFrame, y_train: np.ndarray,
                    games_holdout: pd.DataFrame, p: np.ndarray) -> tuple[dict[str, float], dict[str, float]]:
    score_fit = bt.fit_score_ensemble(
        X_train,
        games_holdout.attrs["train_home_scores"],
        games_holdout.attrs["train_away_scores"],
        "NPB",
    )
    ...
    for i in range(len(games_holdout)):
        lam_h, lam_a = bt.predict_scores(score_fit, X_train.iloc[[0]].iloc[:, :0], "NPB") if False else (0.0, 0.0)
        # Reconstruct the run model prediction with the actual holdout feature row.
        lam_h, lam_a = bt.predict_scores(score_fit, games_holdout.attrs["X_holdout"].iloc[[i]], "NPB")
        ...
        score_choices.append([[int(x.split("-")[0]), int(x.split("-")[1])] if x != "その他" else [-1, -1] for x, _ in choices])
    ...
    score_top4 = float(np.mean([
        (h + a >= 7 and any(pair == [-1, -1] for pair in choices)) or
        (h + a < 7 and [int(h), int(a)] in choices)
    ...
    score = {"ScoreMAE": score_mae, "Top4HitRate": score_top4, "rows": int(len(home_true))}
    return score, hilo
...
def _development_compare(bt: BaseballBacktest, X: pd.DataFrame, y: np.ndarray,
                         start: int, end: int, candidate_names: list[str]) -> dict[str, dict[str, float]]:
    rows: dict[str, list[np.ndarray]] = {name: [] for name in ["ProductionEnsemble"] + candidate_names}
    actual: list[np.ndarray] = []
    for cut in range(start, end, 40):
        stop = min(end, cut + 40)
...
def run_npb_candidate_cycle(
    *,
    ...
    """Produce Development comparison, lock one candidate, and write holdout artifact."""
    ...
    dev_start = max(config.min_train_rows, int(n * (1.0 - config.holdout_fraction - config.dev_fraction)))
    ...
    development = _development_compare(bt, X, y, dev_start, holdout_start, candidate_names)
    ...
    dev_gain = baseline["LogLoss"] - selected_metrics["LogLoss"]
    if dev_gain <= 0:
        return {"stage": "development_evaluated", "decision": "NO_CHANGE", "baseline": baseline, "candidate": selected_metrics, "candidate_model": selected_name, "development": development}

    dataset_hash = hashlib.sha256((json.dumps(development, sort_keys=True) + str(n)).encode("utf-8")).hexdigest()
    spec = CandidateSpec(
        candidate_id="cand-" + hashlib.sha256(f"NPB|{selected_name}|{feature_version}|{git_commit}|{dataset_hash}".encode("utf-8")).hexdigest()[:20],
        league="NPB",
        objective="win",
    ...
    games_holdout = games.iloc[holdout_start:].reset_index(drop=True).copy()
    X_holdout = X.iloc[holdout_start:].reset_index(drop=True)
    games_holdout.attrs["X_holdout"] = X_holdout
    games_holdout.attrs["train_home_scores"] = games.iloc[:holdout_start]["home_score"].astype(float).to_numpy()
    games_holdout.attrs["train_away_scores"] = games.iloc[:holdout_start]["away_score"].astype(float).to_numpy()
    ...
    base_metrics = _metrics(y[holdout_start:], base_p)
    cand_metrics = _metrics(y[holdout_start:], cand_p)
    base_metrics.update(_draw_metrics(y[holdout_start:], base_p))
    cand_metrics.update(_draw_metrics(y[holdout_start:], cand_p))
    base_score, base_hilo = _target_metrics(bt, X_train, y_train, games_holdout, base_p)
    cand_score, cand_hilo = _target_metrics(bt, X_train, y_train, games_holdout, cand_p)
    ...
    (RESULTS / "npb_candidate_development.json").write_text(json.dumps({"development": development, "candidate": asdict(spec), "baseline": baseline}, ensure_ascii=False, indent=2), encoding="utf-8")
    (RESULTS / "npb_locked_holdout.json").write_text(json.dumps(holdout, ensure_ascii=False, indent=2), encoding="utf-8")

...
class BaseballBacktest:
    ...
    def load_npb_pbp(self) -> pd.DataFrame:
        """Load the multi-source compact NPB dataset without double-counting.

        Preferred input is data/2026_multi_source_pbp.csv generated by
        npb_multi_source.py. Legacy monthly *_pbp.csv files remain supported
        as a fallback, but are ignored when the multi-source file exists.
        """
        preferred = self.data_dir / "2026_multi_source_pbp.csv"
        if preferred.exists():
            files = [preferred]
        else:
        ...
        if not files:
            raise FileNotFoundError(
                "NPB data not found. Run npb_multi_source.py first so "
                "data/2026_multi_source_pbp.csv is created."
            )
        chunks = []
        for f in files:
        ...
        if not chunks:
            raise RuntimeError("No readable NPB data files.")
        raw = pd.concat(chunks, ignore_index=True, sort=False)
        out = self._normalize_npb_pbp(raw)
        # One compact row should represent one game. If multiple legacy files
        # contain the same game, deduplicate here before aggregation.
        if "game_id" in out:
            out = out.sort_values(["date","game_id","row_order"]).drop_duplicates(
                ["game_id"], keep="first"
    ...
    def _update_pitcher_history(self, row: pd.Series):
        ...
        for side in ("home", "away"):
            ...
            if isinstance(metrics, dict):
                ...
                continue
            cols = {}
            for m in ("era", "whip", "k9", "bb9", "hr9", "fip"):
